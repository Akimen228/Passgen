...
# Объект бота
bot = Bot(token="2081870256:AAH4owFbDYkMuvdSsUy_MS032PTr_6TNRKA")
admin = 261821892
# Диспетчер для бота
dp = Dispatcher(bot)
...
async def start(message: types.Message):
    ...
    await bot.send_message(user_id, "Привет! Я бот для генерации паролей Мой создатель: @Akimen6", reply_markup=key)
...
async def buttons(message: types.Message):
    ...
    if message.text == 'Сгенерировать':
        ...
        for n in range(number):
            ...
            try:
                from telegram import ParseMode
                await bot.send_message(user_id, "Сгенерированный пароль: <code>{}</code>".format(password), ParseMode.HTML, reply_markup=key)
            except Exception as error:
                await bot.send_message(user_id, "Сгенерированный пароль: ", reply_markup=key)
